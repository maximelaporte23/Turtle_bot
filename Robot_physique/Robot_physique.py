...
class LidarReaderAndMover(Node):
    def __init__(self):
        ...
        self.K0 = 1  # Constante pour la vitesse linéaire
        self.K1 = 2  # Constante pour la vitesse angulaire
        self.target_distance = 0.6
        self.tolerance = 0.02
        self.stability_time = 3.0
    ...
    def calculate_barycenter(self):
        ...
        for i in range(len(ranges)):
            ...
            if 0.5 <= r <= 1.0 and (angle_deg >= 348 or angle_deg <= 12):
                x = r * math.cos(angle)
                y = r * math.sin(angle)
                points.append([x, y])
    ...
    def follow_me(self, cmd):
        ...
        self.get_logger().info(
            f"Cmd: linear_x = {cmd.linear.x:.2f}, angular_z = {cmd.angular.z:.2f}, "
            f"Mode: {self.mode}, x_avg = {x_avg}, y_avg = {y_avg}"
        )
    ...
    def go_home(self, cmd):
        ...
        cmd.linear.x = 0.0
        cmd.angular.z = 0.0
        self.mode = "follow_me"  # Passer au mode go_dock après le retour
